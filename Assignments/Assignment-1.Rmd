...
This R markdown file contains all the code and graphics required for the Assignment-1 of Data Visualization course.
...
The scatter plot between the numerical variables *bill_length_mm* and *flipper_length_mm* from the *penguins* data set 
...
The image below differentiates the three different species in our dataset.
...
Modified the scatter plot by following the below instructions:
<ul>
...
col.code <- c(alpha("red",0.6), alpha("blue",0.6), alpha("green",0.6))
...
legend("topleft", c("Adelie","Gentoo","Chinstrap"), col=c("red","blue","green"), pch = 19)
