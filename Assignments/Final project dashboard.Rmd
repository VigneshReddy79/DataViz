...
### **Box plot showing the distribution of each diamond cut**

```{r}

renderPlot({
   ggplot(data=workDat(), aes(x=workDat()$cut, y=workDat()[[input$Y]])) + 
      geom_boxplot(aes(fill=workDat()$cut)) +
      ylab(input$Y) +
      xlab("Diamond cut") +
      labs(fill = "Cut") +
...
### **Ridge Plot: Distribution of Price($) by diamond cut**

```{r}
renderPlotly({
  if(input$cut == "All"){
    price.fair.len <- dmnd[which(dmnd$cut == "Fair"),]
    fair <- density(price.fair.len$price, na.rm=TRUE)
    price.good.len <- dmnd[which(dmnd$cut == "Good"),]
    good <- density(price.good.len$price, na.rm=TRUE)
    price.vgood.len <- dmnd[which(dmnd$cut == "Very Good"),]
    vgood <- density(price.vgood.len$price, na.rm=TRUE)
    price.ideal.len <- dmnd[which(dmnd$cut == "Ideal"),]
    ideal <- density(price.ideal.len$price, na.rm=TRUE)
    price.premium.len <- dmnd[which(dmnd$cut == "Premium"),]
    premium <- density(price.premium.len$price, na.rm=TRUE)
    
    # plot density curves
    plot_ly(x = ~ideal$x, y = ~ideal$y, 
               type = 'scatter', 
               mode = 'lines', 
               name = 'Ideal', 
               fill = 'tozeroy')  %>% 
